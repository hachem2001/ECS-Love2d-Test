...
local bulletcolor = colorutils:neww(255, 255, 100, 255)
...
function bullets:add(info)
    ...
    -- info.avoid_type
    local body_id, body = ECS:new_component("body", "bullets",
                          global_id, info.pos.x or 0, info.pos.y or 0,
                          info.w or 2, info.h or 2, bulletmass, 0, nil, 0)
    ...
    m.body.vel = (info.direction or error('info.direction vector no given', 2))^bulletspeed + (info.shooter_speed or vector(0,0)); -- length = 1
