...
function npcs:add(info) -- adds the NPCS
	local m = {}
	m.body_id, m.body = ECS:new_component("body", 'npcs', global_id, info.x or 32, info.y or 32, info.w or 32, info.h or 32, info.m, info.friction, info.bounciness)
...
function npcs:update(dt)
	...
	if delay < 0 then
		...
		for k,v in pairs(self.npcs) do
			ECS:new_entity("bullets", {pos = vector:new(v.body.pos[1]+v.body.w/2, v.body.pos[2]+v.body.h/2),
							direction=vector:new(player.body.pos[1] - v.body.pos[1]-v.body.w/2, player.body.pos[2]- v.body.pos[2]-v.body.h/2),
